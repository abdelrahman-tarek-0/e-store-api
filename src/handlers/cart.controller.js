...
const getAllCarts = async (req, res,next) => {
    try {
        ...
        carts.length === 0? res.status(404).json(resBuilder("no carts",404)) : res.status(200).json(resBuilder(carts))
        return
    } catch (error) {
        next(error)
    }
...
const getCart = async (req, res,next) => {
    try {
        ...
        cart? res.json(resBuilder(cart)) : res.status(404).json(resBuilder("no cart",404));
        return
    } catch (error) {
        next(error)
    }
