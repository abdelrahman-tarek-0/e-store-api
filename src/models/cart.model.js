        ...
        try {
            ...
            for (let i = 0; i < itemsArrayNumber.length; i++) {
                if (!productIds.includes(itemsArrayNumber[i])) { 
                    throw new Error(`Product with id: ${itemsArrayNumber[i]}  does not exist`)
                }else{
                    if (itemValues[i] > products.rows.find(product => product.id === itemsArrayNumber[i]).stock) {
                        throw new Error('Not enough stock for product: ' + products.rows.find(product => product.id === itemsArrayNumber[i]).name + ' -- with id ' + itemsArrayNumber[i])
                    }
                }
            }
